...
Usage on production:
    Already copied to ~/beryl3/
    cd ~/beryl3
    python task65_patch_default_image_prod.py
"""
import os
import sys

# Path to models.py on production (Django 1.9 structure)
MODELS_FILE = '/home/mdubiel/beryl3/web/models.py'

# Backup file
BACKUP_FILE = '/home/mdubiel/beryl3/web/models.py.bak-task65'
...
def apply_patch():
    ...
    if old_code in content:
        ...
        if '_prefetched_objects_cache' in content:
            print("\n✅ It looks like the patch is already applied!")
            return True
...
def restart_service():
    ...
    print("   tail -f ~/beryl3/logs/persormance.jsonl")
...
if __name__ == '__main__':
    ...
    if not os.path.exists(MODELS_FILE):
        ...
        print("Make sure you're running this on the production server.")
        print("\nTrying to find models.py...")
        # Try to find it
        for root, dirs, files in os.walk('/home/mdubiel/beryl3'):
            if 'models.py' in files and 'web' in root:
                print(f"Found: {os.path.join(root, 'models.py')}")
        sys.exit(1)
