...
def landing_view(request, landing):
    ...
    if landing is None:
        ...
        logger.error('landing_view: Landing ID is required but was not provided by user %s [%s]',
                    request.user.username if request.user.is_authenticated else 'Anonymous',
                    request.user.id if request.user.is_authenticated else 'None',
                    extra={'function': 'landing_view', 'action': 'landing_view_failed', 
                          'error': 'missing_landing_id', 'result': 'bad_request', 
                          'function_args': {'landing': None}})
    ...
    if not set(landing).issubset(allowed_chars):
        ...
        logger.error('landing_view: Invalid characters in landing ID: "%s" by user %s [%s]',
                    landing, request.user.username if request.user.is_authenticated else 'Anonymous',
                    request.user.id if request.user.is_authenticated else 'None',
                    extra={'function': 'landing_view', 'action': 'landing_view_failed', 
                          'landing_id': landing, 'error': 'invalid_format', 'result': 'bad_request',
    ...
    try:
        ...
        logger.error('landing_view: Landing page template not found for ID: "%s" by user %s [%s]',
                    landing, request.user.username if request.user.is_authenticated else 'Anonymous',
                    request.user.id if request.user.is_authenticated else 'None',
                    extra={'function': 'landing_view', 'action': 'landing_view_failed', 
                          'landing_id': landing, 'template': template_name, 'error': 'template_not_found',
    ...
    logger.info('landing_view: Landing page "%s" accessed successfully by user %s [%s]',
               landing, request.user.username if request.user.is_authenticated else 'Anonymous',
               request.user.id if request.user.is_authenticated else 'None',
               extra={'function': 'landing_view', 'action': 'landing_view', 'landing_id': landing,
                     'template': template_name, 'result': 'success', 'function_args': {'landing': landing}})
